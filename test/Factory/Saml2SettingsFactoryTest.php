<?php

namespace MehrAlsNix\ZFTest\SAML\Factory;

use MehrAlsNix\ZF\SAML\Factory\Saml2SettingsFactory;
use Zend\ServiceManager\ServiceManager;
use Zend\Test\PHPUnit\Controller\AbstractHttpControllerTestCase;

class Saml2SettingsFactoryTest extends AbstractHttpControllerTestCase
{
    /**
     * @var Saml2SettingsFactory
     */
    protected $factory;
    /**
     * @var ServiceManager
     */
    protected $services;
    protected function setUp()
    {
        $this->factory = new Saml2SettingsFactory();
        $this->services = $services = new ServiceManager();
        $this->setApplicationConfig([
            'modules' => [
                'MehrAlsNix\ZF\SAML',
            ],
            'module_listener_options' => [
                'module_paths' => [__DIR__ . '/../../'],
                'config_glob_paths' => [],
            ],
            'service_listener_options' => [],
            'service_manager' => [],
        ]);
        parent::setUp();
    }

    /**
     * @test
     */
    public function instantiation()
    {
        $this->services->setService('Config', [
            'zf-saml' => [
                'sp' =>  [
                    'entityId' => 'http://stuff.com/endpoints/metadata.php',
                    'assertionConsumerService' =>  [
                        'url' => 'http://stuff.com/endpoints/endpoints/acs.php',
                    ],
                    'singleLogoutService' =>  [
                        'url' => 'http://stuff.com/endpoints/endpoints/sls.php',
                    ],
                    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:unspecified',
                ],
                'idp' =>  [
                    'entityId' => 'http://idp.example.com/',
                    'singleSignOnService' =>  [
                        'url' => 'http://idp.example.com/SSOService.php',
                    ],
                    'singleLogoutService' =>  [
                        'url' => 'http://idp.example.com/SingleLogoutService.php',
                    ],
                    'x509cert' => '',
                ],
            ],
        ]);
        $expectedService = new \OneLogin_Saml2_Settings(
            [
                'sp' =>  [
                    'entityId' => 'http://stuff.com/endpoints/metadata.php',
                    'assertionConsumerService' =>  [
                        'url' => 'http://stuff.com/endpoints/endpoints/acs.php',
                    ],
                    'singleLogoutService' =>  [
                        'url' => 'http://stuff.com/endpoints/endpoints/sls.php',
                    ],
                    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:unspecified',
                ],
                'idp' =>  [
                    'entityId' => 'http://idp.example.com/',
                    'singleSignOnService' =>  [
                        'url' => 'http://idp.example.com/SSOService.php',
                    ],
                    'singleLogoutService' =>  [
                        'url' => 'http://idp.example.com/SingleLogoutService.php',
                    ],
                    'x509cert' => '',
                ]
            ]
        );

        $service = $this->factory->createService($this->services);
        $this->assertInstanceOf('OneLogin_Saml2_Settings', $service);
        $this->assertEquals($expectedService, $service);
    }
}
